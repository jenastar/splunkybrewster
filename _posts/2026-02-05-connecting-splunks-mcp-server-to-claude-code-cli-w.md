---
layout: post
title: 'Connecting Splunk''s MCP Server to Claude Code CLI: What Actually Worked (and
  What Didn''t)'
...
1. **Node.js and NPX installed** on the machine making the MCP calls
   ```bash
   which npx
   ```
   If nothing comes back, install Node.js. In WSL: `sudo apt update && sudo apt install nodejs npm`
...
3. **Create a token** for that user:
   - Go to **Settings > Tokens** in Splunk Web
   - Select the correct user
   - Set the audience to **`mcp`** - this is the one people miss. It must say `mcp`, not `search`, not blank.
4. **Copy the token** - carefully. Long tokens and terminals don't mix. If your terminal wraps the token across multiple lines, the MCP proxy will silently reject it with a `Warning: ignoring invalid header argument` that's easy to miss. Paste it directly into your config file in an editor instead.
...
- **If you need to bypass SSL** (Splunk Enterprise with self-signed certs), add an `env` block:
  ```json
  "env": {
...
*Tools/Versions: Splunk Cloud v9.3.2411.123, Splunk MCP Server v1.0.0, Splunk AI Assistant for SPL v1.5.0, mcp-remote (latest via npx), WSL2 on Windows, Claude Code (Opus 4.6)*
