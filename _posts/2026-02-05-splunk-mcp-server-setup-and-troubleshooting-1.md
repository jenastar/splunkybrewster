...
date: '2026-02-05T21:00:00-07:00'
tags:
- splunk
- ai
- mcp
- saia
permalink: /2026/02/splunk-mcp-server-setup-and_0772775727.html
---
...
> **Don't wrap `npx` with `wsl` if you're already in WSL.** If you're already running inside WSL, the `wsl` wrapper double-hops and the proxy won't start. Windows-native apps like Claude Desktop *do* need the `wsl` wrapper. Know where you are.
...
Claude Desktop is a Windows-native app. It can't see `npx` inside WSL without the `wsl` wrapper. Paste the config from the MCP app into Claude Desktop's settings, then wrap the command:
...
If you're already inside WSL, do not use the `wsl` wrapper. You're already there.
